...
export function findCalmestWindow(forecast, location, { now = Date.now(), source, quality } = {}) {
  ...
  for (let index = 0; index < hours.length - 2; index += 1) {
    ...
    if (points.some((point, offset) => point.isDay !== true
      ...
      || getSafety(point, location, (key) => key, 'en-GB', { source, quality }).level !== 'good')) continue
