...
export default function SwimAssessment({ current, location, quality, source, locale, t }) {
  ...
  const date = new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeZone: 'Europe/London' }).format(new Date(`${CATALOG_UPDATED_ON}T12:00:00Z`))
  return (
    <section className="assessment" aria-label={t('outlook.factors')}>
      <div className={`assessment-factor ${wave}`}><Waves size={18} /><div><strong>{t('outlook.waves')}</strong><span>{t(`outlook.${wave}`)}</span></div></div>
