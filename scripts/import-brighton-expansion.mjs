...
if (apply) {
  ...
  await database('data_sources',{method:'POST',prefer:'resolution=ignore-duplicates',body:[source]})
  await database('source_targets',{method:'POST',prefer:'resolution=ignore-duplicates',body:rows.map(({target})=>({id:target.id,source_id:target.source_id,external_id:target.external_id,label:target.label,coverage_type:target.coverage_type,config:target.config}))})
  ...
  await database('location_sources',{method:'POST',prefer:'resolution=ignore-duplicates',body:rows.map(({target,...binding})=>({...binding,target_id:target.id}))})
  let applied=0
  for(const {row,orientation:o} of orientations){
    const params=new URLSearchParams({id:`eq.${row.id}`,active:'eq.true',water_type:'eq.coastal',latitude:`eq.${row.latitude}`,longitude:`eq.${row.longitude}`,metadata:`eq.${JSON.stringify(row.metadata)}`})
